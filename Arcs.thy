...
proof -
  ...
  (* Left neighbors are of size arc_size-1 *)
  ...
  proof -
    ...
    next
      ...
      have "x < length cycle" unfolding f_def using \<open>x = f shift\<close> cycle_pos
        using f_def mod_less_divisor by presburger    
      have "(x + shift) mod length cycle = element_index"
  ...
  (* Right neighbors are of size arc_size-1 *)
  ...
  show "element_arc \<notin> left_neighbors \<and> element_arc \<notin> right_neighbors"
  proof
  ...
  proof (rule equals0I)
    ...
    have r_idx_bound: "r_idx < length cycle"
      using r_idx_def cycle_size cycle_pos by fastforce
    ...
    have "l_idx = r_idx"
      using \<open>generate_n_arc cycle l_idx arc_size = generate_n_arc cycle r_idx arc_size\<close>
            \<open>l_idx < length cycle\<close> r_idx_bound unique_index_unique_arc by metis

    have "hd element_arc = cycle ! element_index"
      unfolding assms(3) generate_n_arc_def
      using minimal_arc_size cycle_size el_idx_bound hd_rotate_conv_nth hd_take list.size(3)
      by (metis add.commute add.right_neutral
          arc_size canonically_ordered_monoid_add_class.lessE cycle_pos mod_less)
 ...
 \<longleftrightarrow> ((element_index + r_shift + l_shift) mod length cycle = element_index)" by presburger
    ...
    consider (no_wrap) "element_index + total_shift < length cycle" | 
             (wrap) "element_index + total_shift \<ge> length cycle" by linarith
    then show False
    proof cases
      case no_wrap
  ...
  proof (rule set_eqI, rule iffI)
    ...
    have el_idx_bound: "element_index < length cycle"
      using assms(1,2) index_limit
      by (metis le_antisym linorder_not_le not_contains_impl_not_elem not_in_list)
    have cycle_pos: "length cycle > 0" using cycle_size by auto
    ...
    have "(l_idx + l_shift) mod length cycle = element_index"
      using hd_match \<open>hd element_arc = cycle ! element_index\<close>
            cycle_elements_distinct el_idx_bound cycle_pos mod_less_divisor nth_eq_iff_index_eq
      by metis
  ...
  next
    ...
    have "l_idx \<in> left_neighbor_indices"
      ...
      using \<open>l_idx < length cycle\<close> \<open>0 < l_shift\<close> \<open>l_shift < arc_size\<close>
            \<open>cycle ! ((l_idx + l_shift) mod length cycle) = hd element_arc\<close> by blast
  ...
  next
    ...
    proof (rule linorder_cases[of i j])
      ...
      have "element_index mod length cycle = other_idx mod length cycle"
        using LHS RHS \<open>((element_index + i) mod length cycle + K) 
        mod length cycle = ((other_idx + i) mod length cycle + K) mod length cycle\<close> by simp
    ...
    next
      ...
      hence RHS: "((other_idx + j) mod length cycle + K) mod length cycle = other_idx mod length cycle"
        using \<open>j + K = length cycle\<close> by (metis add.assoc mod_add_self2)
      ...
      have "other_idx \<in> right_neighbor_indices"
        using \<open>0 < shift\<close> \<open>shift < arc_size\<close> \<open>other_idx = (element_index + shift) mod length cycle\<close> assms(4) by blast
      thus ?thesis using other_arc_def assms(5) by blast
    ...
    next
      ...
      have "(element_index + i) mod length cycle = (other_idx + shift + i) mod length cycle"
        using idx_eq \<open>j = i + shift\<close> by presburger
      ...
      also have "... = hd element_arc"
        unfolding assms(3) generate_n_arc_def using minimal_arc_size cycle_size el_bound hd_rotate_conv_nth hd_take list.size(3)
        by (metis arc_size less_nat_zero_code mod_less)
      ...
      thus ?thesis using other_arc_def assms(8) by blast
    qed
  qed
qed
    ...
    next
      ...
      have "left_neighbors \<inter> right_neighbors = {}"
        using neighbors[of element arc_index element_arc right_neighbor_indices right_neighbors
            ...
            \<open>other_arc \<in> set arcs\<close> by fastforce
      ...
      have "element_arc \<notin> left_arc_neighbors \<and> element_arc \<notin> right_arc_neighbors" 
        using neighbors[of element arc_index element_arc right_neighbor_indices right_neighbors
          ...
          right_neighbors_def \<open>right_neighbor_arc \<in> right_neighbors\<close> left_neighbor_indices_def
          left_neighbors_def \<open>left_neighbor_arc \<in> left_neighbors\<close> \<open>element_arc \<noteq> other_arc\<close>
          left_arc_neighbors_def right_arc_neighbors_def \<open>other_arc \<in> set arcs\<close> by fastforce
        ...
        next
          ...
          have "x \<in> left_neighbors \<or> x \<in> right_neighbors"
            using neighbors[of element arc_index element_arc right_neighbor_indices right_neighbors 
                  ...
                  \<open>hd element_arc \<in> set cycle\<close>[folded element_def] 
                  arc_index_def
                  ...
                  \<open>intersecting_lists element_arc x\<close> by blast
      ...
      have "card left_neighbors = arc_size - 1" using neighbors[of element arc_index element_arc
            ...
            right_neighbors_def \<open>right_neighbor_arc \<in> right_neighbors\<close> left_neighbor_indices_def
            left_neighbors_def \<open>left_neighbor_arc \<in> left_neighbors\<close> \<open>element_arc \<noteq> other_arc\<close> 
            \<open>other_arc \<in> set arcs\<close> by argo
       ...
       have "left_arc \<in> left_arc_neighbors"
        ...
        proof
          ...
          have len_left: "length left_arc = arc_size" 
            ...
            by (metis IntE \<open>left_arc \<in> left_arc_neighbors\<close> fixed_arc_size left_arc_neighbors_def
                n_arc_of_cycle_def)
